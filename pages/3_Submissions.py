...
st.markdown("""
            Each submission should be a `.zip` file named **only** by the **team leader's email** (such as `xxx@gmail.com.zip`), which contains two folders, one for the runnable source code of task1 and the other for task2 (if necessary, include `requirement.txt`), and both should include a function `evaluation` for testing with the following input and output formats:
            + Input: 10000 three channel facial images of different sizes from Sketch domain.
            + Output: The metric result obtained from trained classifiers (Accuracy for task 1; Accuracy and $\Delta{DP}$ for task 2).

            At the end of each stage, we will test the team's last submission on our **test set** and publish the results on *Leaderboard* within a week.
...
st.info(f"Each submission should be a `.zip` file named **only** by the **team leader's email** (such as `xxx@gmail.com.zip`)")

st.markdown("Please use the following link to submit the file:")
st.markdown("https://docs.google.com/forms/d/e/1FAIpQLSftB0ZA_rFMuVLkrM8HWRPyjGauXiCtmHfwHBf5dw5E27FfIw/viewform?usp=sf_link")
